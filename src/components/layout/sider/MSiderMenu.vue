<template>
  <a-layout-sider class="cs-sider" v-model:collapsed="collapsed" defaultCollapsed collapsible mode="inline">
    ...
    <a-menu v-model:openKeys="openKeys" v-model:selectedKeys="selectedKeys" mode="vertical" @click="handleClick" theme="dark" class="cs-sider-menu">
      <a-menu-item key="1">
        <template #icon>
          <MailOutlined />
        </template>
...
const loadTenantMenus = () => {
  proxy.api.grant.tenantMenus().then((res:any) => {
    menuList.value = res.data;
    const dourt = treeFindPath(menuList.value, (data:any) => data.name === route.name, "id");
    const leng:any = dourt.pop();
...
const menuclick = (e: any) => {
  ...
  if (router.hasRoute(e.item.name)) {
    ...
    router.push({ name: "workflow-error-404" });
  }
};
