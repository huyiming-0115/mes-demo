<template>
  ...
  <div style="padding-left: 5px;">
    ...
    >
      <!-- 部门 -->
      <a-form-item class="form-item-require" label="装潢名称" name="name" :rules="rules.name">
        <a-input placeholder="装潢名称" style="width: 100%;" v-model:value="formState.name" />
      </a-form-item>
      <!-- 岗位 -->
      <a-form-item class="form-item-require" label="装潢设置" name="title">
        <span class="btn-link" @click="setDressFn">点击设置</span>
      </a-form-item>
    </a-form>
  ...
  <MDialog :dialog="dialogs">
    <DetailSellDress @close="dialogs.show = false" :dialog="dialogs" :pid="dialogs.flag" :row="dialogs.row"></DetailSellDress>
  </MDialog>
</template>
